...
% Place for alpha
Toma.OtherTemporalVars.OMA_alpha = NaN( [ size(m.ux_tri,2), ...
                    numel(p.TimeStamp) ] );
if nargout > 1
  OMA_cov_alpha = NaN( [ size(m.ux_tri,2)^2, numel(p.TimeStamp) ] );
end
...
for i = 1:length(p.TimeStamp)
  ...
  for j = 1:numel(R)
    ...
    uux = [ uux; repmat( ux{j}, [ length(tt), 1 ] ) ]; %#ok<AGROW> 
    uuy = [ uuy; repmat( uy{j}, [ length(tt), 1 ] ) ]; %#ok<AGROW> 
    head = [ head; repmat( R(j).RangeBearHead(:,3), [ length(tt), 1 ] ) ]; %#ok<AGROW> 

    ss = R(j).RadComp(:,tt);
    speed = [ speed; ss(:) ]; %#ok<AGROW> 
