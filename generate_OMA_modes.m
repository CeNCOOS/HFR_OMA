...
% Start saving
[aa,bb] = fileparts( fn ); %#ok<ASGLU> 
[cc,bb] = fileparts( tempname ); %#ok<ASGLU> 
...
for k = 1:length(ob_nums)
  ...
  max_bound_modes(k,1) = calculate_max_num_boundary_modes( ob_dd, md ); %#ok<AGROW> 
  ...
  if exist( 'bound_modes', 'var' )
    bound_modes = [ bound_modes, bb ]; %#ok<AGROW> 
  else
    bound_modes = bb;
  end
...
% us.
global MAP_PROJECTION MAP_VAR_LIST MAP_COORDS %#ok<GVMIS> 
